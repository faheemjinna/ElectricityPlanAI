...
def processEnergyEstimates(typeInput, companyInput, usageInputArray, loadLatest):
    ...
    plan_data = {}  # key = planName, value = [month1_estimate, ..., month12_estimate]

    if loadLatest:
        print("🔄 Fetching and processing new files...")
        # Step 2A: Fetch and process new files
        ...
        for file_info in downloaded_files:
            ...
            try:
                ...
                monthly_estimates = [
                    round(formulaLogic.evaluateFormula(usage, base, formula), 2)
                    for usage in usageInputArray
                ]
        ...
        # Step 2B: Load existing plan details from DB
        mycursor.execute("""
            SELECT pd.planID, pd.plan_name, p.baseAmount, p.formula
            FROM plan_details pd
            JOIN plans p ON pd.planID = p.planID
            WHERE pd.companyID = %s AND pd.type = %s
        """, (companyInput, typeInput))
    ...
    for month_idx in range(12):
        row = [f"Month {month_idx + 1}", usageInputArray[month_idx]]
    ...
    # Step 4: Print lowest total plan
    if total_costs:
